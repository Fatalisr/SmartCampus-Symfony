...
class referentControllerTest extends WebTestCase
{
    ...
    public function testReferentPage() // Test de l'affichage de la page d'accueil du référent
    {
        ...
        $form = $crawler->filter('#form_changer_salle')->form();
    ...
    public function testviewSAInterventionPage()
    {
        // Se connecte en temps que Referent
        //=============================================================
        $client = static::createClient();
...
//Test Room
        ...
        $client->request('GET', '/referent/saInt/'.$sa->getId());
    ...
    public function testChangerMessageSAIntervention()
    {
        // Se connecte en temps que Referent
        //=============================================================
        $client = static::createClient();
        ...
        $crawler = $client->request('GET', '/referent/saInt/'.$sa->getId());
        // Delete the entry in the database to avoid conflict with the tests
        $entityManager = $client->getContainer()->get('doctrine.orm.entity_manager');

        $form = $crawler->selectButton('CHANGER LE MESSAGE')->form([
            'maintenance_form[report]' => 'test'
        ]);
        $client->submit($form);
        $this->assertResponseRedirects('/referent');
        ...
        $sa = $saRepository->findOneBy(['name' => 'Test_SA']);
        $this->assertInstanceOf(SA::class, $sa);
        $this->assertSame('ACTIF', $sa->getState()); // Assuming this is the expected state
